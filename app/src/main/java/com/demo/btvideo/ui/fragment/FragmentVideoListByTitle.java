...
//作品页面
public class FragmentVideoListByTitle extends Fragment {
	...
	private static class Holder {
		private static FragmentVideoListByTitle instance = new FragmentVideoListByTitle();
	}

	public static FragmentVideoListByTitle getInstance() {
		return FragmentVideoListByTitle.Holder.instance;
	}
...
//				if (pair.second==0) {
					dataLoaderViewModel.getUploadVideoT(pair.first).removeObservers(getViewLifecycleOwner());
					dataLoaderViewModel.getUploadVideoT(pair.first).observe(getViewLifecycleOwner(), collectionPagingData -> {
