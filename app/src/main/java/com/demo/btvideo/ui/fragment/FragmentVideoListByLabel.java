...
//作品页面
public class FragmentVideoListByLabel extends Fragment {
	...
	private static class Holder {
		private static FragmentVideoListByLabel instance = new FragmentVideoListByLabel();
	}

	public static FragmentVideoListByLabel getInstance() {
		return FragmentVideoListByLabel.Holder.instance;
	}
...
//				if (pair.second==3) {
					dataLoaderViewModel.getUploadVideoL(pair.first).removeObservers(getViewLifecycleOwner());
					dataLoaderViewModel.getUploadVideoL(pair.first).observe(getViewLifecycleOwner(), collectionPagingData -> {
